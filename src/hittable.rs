...
impl Hittable for HittableList {
    ...
    fn random(&self, origin: &Point3) -> Vec3 {
        ...
        let idx = random_int(0, (int_size - 1) as i32) as usize;
...
// 用于将 &dyn Hittable 包装为实现 Hittable trait 的结构体，便于 Arc/Pdf 混合采样
pub struct HittableRefWrapper<'a> {
    pub inner: &'a dyn Hittable,
}

impl<'a> HittableRefWrapper<'a> {
    pub fn new(inner: &'a dyn Hittable) -> Self {
        Self { inner }
    }
}

impl<'a> Hittable for HittableRefWrapper<'a> {
    fn hit(
        &self,
        r: &Ray,
        ray_t: Interval,
