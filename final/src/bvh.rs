...
impl BvhNode {
    ...
    pub fn new(objects: &mut [Arc<dyn Hittable + Send + Sync>], start: usize, end: usize) -> Self {
        ...
        let (left, right): (Arc<dyn Hittable + Send + Sync>, Arc<dyn Hittable + Send + Sync>) = if object_span == 1 {
            ...
            // 注意：这里不能使用 rayon::join 因为两个闭包需要同时访问同一个可变数组
            // 而 Rust 的借用检查器不允许这种情况
            let left_node = BvhNode::new(objects, start, mid);
            let right_node = BvhNode::new(objects, mid, end);
            
            (Arc::new(left_node), Arc::new(right_node))
        };
