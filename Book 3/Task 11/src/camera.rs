...
impl Camera {
    ...
    fn ray_color(&self, r: &Ray, depth: usize, world: &dyn Hittable, lights: &dyn Hittable) -> Color {
        // 射线反弹深度限制
        if depth <= 0 {
            return self.background; // 递归终止时返回背景色
        }
        ...
        if world.hit(r, Interval::new(0.001, f64::INFINITY), &mut rec) {
            let mat = match &rec.mat {
                Some(m) => m,
                None => return self.background, // 没有材质时返回背景色
            };

            let mut scattered = Ray::default();
            let mut attenuation = Color::default();
            let mut pdf_value = 0.0f64;
            let mut rng = thread_rng();

            // 发光材质直接返回自发光
            let color_from_emission = mat.emitted(rec.u, rec.v, &rec.p);
            if !mat.scatter(r, &rec, &mut attenuation, &mut scattered, &mut pdf_value, &mut rng) {
            ...
            use crate::pdf::{HittablePdf, CosinePdf, MixturePdf, Pdf};
            use std::sync::Arc;
            // 用 HittableList 包装 lights 引用，避免类型不匹配
            let mut lights_list = crate::hittable::HittableList::new();
            lights_list.add(Arc::new(crate::hittable::Translate::new(
                Arc::new(crate::hittable::RotateY::new(Arc::new(crate::hittable::HittableList::with_object(Arc::new(crate::hittable::HittableList::new()))), 0.0)),
                crate::vec3::Vec3::new(0.0, 0.0, 0.0),
            )));
            let lights_arc: Arc<dyn Hittable + Send + Sync> = Arc::new(lights_list);
            let p0 = Arc::new(HittablePdf::new(lights_arc, rec.p));
